DROP TABLE pfsawh_item_sn_l_fact;
...
/*--*----|----*----|----*----|----*----|----*----|----*----|----*----|----*---*/
--
CREATE TABLE pfsawh_item_sn_l_fact
    (
...
ALTER TABLE pfsawh_item_sn_l_fact  
...
ALTER TABLE pfsawh_item_sn_l_fact  
    DROP CONSTRAINT fk_pfsawh_item_sn_l_fact_pbaid;        

ALTER TABLE PFSAWH.pfsawh_item_sn_l_fact 
    ADD CONSTRAINT fk_pfsawh_item_sn_l_fact_pbaid 
    FOREIGN KEY 
        (
        pba_id
        ) 
...
COMMENT ON COLUMN pfsawh_item_sn_l_fact.life_nmcm_int_hrs 
...
COMMENT ON COLUMN pfsawh_item_sn_l_fact.life_nmcm_dep_hrs 
...
COMMENT ON COLUMN pfsawh_item_sn_l_fact.life_nmcs_int_hrs 
...
COMMENT ON COLUMN pfsawh_item_sn_l_fact.life_nmcs_dep_hrs 
...
COMMENT ON COLUMN pfsawh_item_sn_l_fact.life_pmcm_hrs 
IS 'LIFE_PMCM_HRS - The total number of hours in a partial mission capable maintenance status during its lifetime.';

COMMENT ON COLUMN pfsawh_item_sn_l_fact.life_pmcm_user_hrs 
...
COMMENT ON COLUMN pfsawh_item_sn_l_fact.life_pmcm_int_hrs 
...
COMMENT ON COLUMN pfsawh_item_sn_l_fact.life_pmcs_int_hrs 
...
COMMENT ON COLUMN pfsawh_item_sn_l_fact.life_maint_action_cnt 
IS 'LIFE_MAINT_ACTION_CNT -  The number of maintenance actions that have been identified during its lifetime';
 
COMMENT ON COLUMN pfsawh_item_sn_l_fact.life_operat_readiness_rate 
IS 'LIFE_OPERAT_READINESS_RATE -  during its lifetime';

COMMENT ON COLUMN pfsawh_item_sn_l_fact.life_operat_cost_per_hour 
IS 'LIFE_OPERAT_COST_PER_HOUR -  during its lifetime';

COMMENT ON COLUMN pfsawh_item_sn_l_fact.life_cost_parts 
IS 'LIFE_COST_PARTS -  during its lifetime';

COMMENT ON COLUMN pfsawh_item_sn_l_fact.life_cost_manpower 
IS 'LIFE_COST_PARTS -  during its lifetime';

COMMENT ON COLUMN pfsawh_item_sn_l_fact.life_deferred_maint_items 
IS 'LIFE_DEFERRED_MAINT_ITEMS -  during its lifetime';

COMMENT ON COLUMN pfsawh_item_sn_l_fact.life_operat_hrs_since_lst_ovhl 
IS 'LIFE_OPERAT_HRS_SINCE_LST_OVHL -  during its lifetime';

COMMENT ON COLUMN pfsawh_item_sn_l_fact.life_maint_hrs_since_lst_ovhl
IS 'LIFE_MAINT_HRS_SINCE_LST_OVHL -  during its lifetime';

COMMENT ON COLUMN pfsawh_item_sn_l_fact.life_time_since_lst_ovhl 
IS 'LIFE_TIME_SINCE_LST_OVHL -  during its lifetime';

COMMENT ON COLUMN pfsawh_item_sn_l_fact.notes 
IS 'NOTES - Processing notes from the ETL process.'; 

COMMENT ON COLUMN pfsawh_item_sn_l_fact.status 
IS 'STATUS - The status of the record in question.';

COMMENT ON COLUMN pfsawh_item_sn_l_fact.updt_by 
IS 'UPDT_BY - The date/timestamp of when the record was created/updated.';

COMMENT ON COLUMN pfsawh_item_sn_l_fact.lst_updt 
IS 'LST_UPDT - Indicates either the program name or user ID of the person who updated the record.';

COMMENT ON COLUMN pfsawh_item_sn_l_fact.active_flag 
IS 'ACTIVE_FLAG - Flag indicating if the record is active or not.';

COMMENT ON COLUMN pfsawh_item_sn_l_fact.active_date 
IS 'ACTIVE_DATE - Addition control for active_Fl indicating when the record became active.';

COMMENT ON COLUMN pfsawh_item_sn_l_fact.inactive_date 
IS 'INACTIVE_DATE - Addition control for active_Fl indicating when the record went inactive.';

COMMENT ON COLUMN pfsawh_item_sn_l_fact.insert_by 
IS 'INSERT_BY - Reports who initially created the record.';

COMMENT ON COLUMN pfsawh_item_sn_l_fact.insert_date 
IS 'INSERT_DATE - Reports when the record was initially created.';

COMMENT ON COLUMN pfsawh_item_sn_l_fact.update_by 
IS 'UPDATE_BY - Reports who last updated the record.';

COMMENT ON COLUMN pfsawh_item_sn_l_fact.update_date 
IS 'UPDATE_DATE - Reports when the record was last updated.';

COMMENT ON COLUMN pfsawh_item_sn_l_fact.delete_flag 
IS 'DELETE_FLAG - Flag indicating if the record can be deleted.';

COMMENT ON COLUMN pfsawh_item_sn_l_fact.delete_date 
IS 'DELETE_DATE - Addition control for DELETE_FLAG indicating when the record was marked for deletion.';

COMMENT ON COLUMN pfsawh_item_sn_l_fact.hidden_flag 
IS 'HIDDEN_FLAG - Flag indicating if the record should be hidden from the general user in things like drop-down lists.';

COMMENT ON COLUMN pfsawh_item_sn_l_fact.hidden_date 
IS 'HIDDEN_DATE - Addition control for HIDDEN_FLAG indicating when the record was hidden.';

COMMENT ON COLUMN pfsawh_item_sn_l_fact.etl_processed_by 
...
WHERE  table_name = UPPER('pfsawh_item_sn_l_fact'); 
...
LEFT OUTER JOIN user_tab_columns b ON b.table_name = UPPER('pfsawh_item_sn_l_fact') 
    AND  a.column_name = b.column_name
WHERE    a.table_name = UPPER('pfsawh_item_sn_l_fact');  
