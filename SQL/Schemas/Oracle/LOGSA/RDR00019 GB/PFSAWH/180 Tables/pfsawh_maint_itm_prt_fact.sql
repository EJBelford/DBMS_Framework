/*--*----|----*----|----*----|---- TEAM ITSS ----*----|----|----*----|----*---*/
--
--         NAME: pfsawh_maint_itm_prt_fact
--      PURPOSE: To calculate the desired information.
--
-- TABLE SOURCE: pfsawh_maint_itm_prt_fact.sql 
--
...
DROP TABLE pfsawh_maint_itm_prt_fact;
...
COMMENT ON TABLE pfsawh_maint_itm_prt_fact 
...
COMMENT ON COLUMN pfsawh_maint_itm_prt_fact.rec_id
IS 'REC_ID - Sequence/identity for dimension/fact table.'; 

COMMENT ON COLUMN pfsawh_maint_itm_prt_fact.tsk_end_date_id 
IS 'TSK_END_DATE_ID - Foreign key to the PFSAWH_DATE_DIM records.'; 

COMMENT ON COLUMN pfsawh_maint_itm_prt_fact.tsk_end_time_id 
IS 'TSK_END_TIME_ID - Foreign key to the PFSAWH_DATE_DIM records.'; 

COMMENT ON COLUMN pfsawh_maint_itm_prt_fact.physical_item_id 
IS 'PHYSICAL_ITEM_ID - Foreign key to the PFSAWH_ITEM_DIM table.'; 
     
COMMENT ON COLUMN pfsawh_maint_itm_prt_fact.physical_item_sn_id 
IS 'PHYSICAL_ITEM_SN_ID - Foreign key to the PFSAWH_ITEM_SN_DIM table.'; 

COMMENT ON COLUMN pfsawh_maint_itm_prt_fact.item_comp_id 
IS 'ITEM_COMP_ID - Foreign key to the PFSAWH_ITEM_DIM table.';   

COMMENT ON COLUMN pfsawh_maint_itm_prt_fact.item_part_id 
IS 'ITEM_PART_ID - Foreign key to the PFSAWH_ITEM_DIM table.';     

COMMENT ON COLUMN pfsawh_maint_itm_prt_fact.item_comp_sn 
IS 'ITEM_COMP_SN - ';   

COMMENT ON COLUMN pfsawh_maint_itm_prt_fact.part_num 
...
COMMENT ON COLUMN pfsawh_maint_itm_prt_fact.tsk_elapsed_part_wt_tm 
...
COMMENT ON COLUMN pfsawh_maint_itm_prt_fact.tsk_was_def_flag 
IS 'TSK_WAS_DEF_FLAG - Flag indicating if this task was the result of a defect.'; 

COMMENT ON COLUMN pfsawh_maint_itm_prt_fact.tsk_sched_flag 
IS 'TSK_SCHED_FLAG - SCHED_UNSCHED - Flag indicating if this task was a scheduled or un-scheduled. Values are ?\S\U'; 

COMMENT ON COLUMN pfsawh_maint_itm_prt_fact.status 
IS 'STATUS - The status of the record in question.';

COMMENT ON COLUMN pfsawh_maint_itm_prt_fact.updt_by 
IS 'UPDT_BY - The date/timestamp of when the record was created/updated.';

COMMENT ON COLUMN pfsawh_maint_itm_prt_fact.lst_updt 
IS 'LST_UPDT - Indicates either the program name or user ID of the person who updated the record.';

COMMENT ON COLUMN pfsawh_maint_itm_prt_fact.active_flag 
IS 'ACTIVE_FLAG - Flag indicating if the record is active or not.';

COMMENT ON COLUMN pfsawh_maint_itm_prt_fact.active_date 
IS 'ACTIVE_DATE - Additional control for active_Fl indicating when the record became active.';

COMMENT ON COLUMN pfsawh_maint_itm_prt_fact.inactive_date 
IS 'INACTIVE_DATE - Additional control for active_Fl indicating when the record went inactive.';

COMMENT ON COLUMN pfsawh_maint_itm_prt_fact.insert_by 
IS 'INSERT_BY - Reports who initially created the record.';

COMMENT ON COLUMN pfsawh_maint_itm_prt_fact.insert_date 
IS 'INSERT_DATE - Reports when the record was initially created.';

COMMENT ON COLUMN pfsawh_maint_itm_prt_fact.update_by 
IS 'UPDATE_BY - Reports who last updated the record.';

COMMENT ON COLUMN pfsawh_maint_itm_prt_fact.update_date 
IS 'UPDATE_DATE - Reports when the record was last updated.';

COMMENT ON COLUMN pfsawh_maint_itm_prt_fact.delete_flag 
IS 'DELETE_FLAG - Flag indicating if the record can be deleted.';

COMMENT ON COLUMN pfsawh_maint_itm_prt_fact.delete_date 
IS 'DELETE_DATE - Additional control for DELETE_FLAG indicating when the record was marked for deletion.';

COMMENT ON COLUMN pfsawh_maint_itm_prt_fact.hidden_flag 
IS 'HIDDEN_FLAG - Flag indicating if the record should be hidden from the general user in things like drop-down lists.';

COMMENT ON COLUMN pfsawh_maint_itm_prt_fact.hidden_date 
IS 'HIDDEN_DATE - Addition control for HIDDEN_FLAG indicating when the record was hidden.';

COMMENT ON COLUMN pfsawh_maint_itm_prt_fact.etl_processed_by 
...
WHERE    table_name = UPPER('pfsawh_maint_itm_prt_fact'); 
...
LEFT OUTER JOIN user_tab_columns b ON b.table_name = UPPER('pfsawh_maint_itm_prt_fact') 
    AND a.column_name = b.column_name
WHERE    a.table_name = UPPER('pfsawh_maint_itm_prt_fact') 
...
DROP INDEX idx_pfsawh_maint_itm_prt_fact;

CREATE INDEX idx_pfsawh_maint_itm_prt_fact 
    ON pfsawh.pfsawh_maint_itm_prt_fact
    (
    physical_item_id,        
    physical_item_sn_id,        
    tsk_end_date_id,
...
ALTER TABLE pfsawh_maint_itm_prt_fact  
...
SELECT * FROM pfsawh_maint_itm_prt_fact; 
