...
DROP SEQUENCE pfsawh_process_log_seq;

CREATE SEQUENCE pfsawh_process_log_seq
    START WITH 1
--    MAXVALUE 9999
    MINVALUE 1
    NOCYCLE
...
COMMENT ON TABLE pfsawh_process_log 
...
COMMENT ON COLUMN pfsawh_process_log.process_key 
IS 'Identity and Primary key for facRptToCourtProcessLog records.'; 

COMMENT ON COLUMN pfsawh_process_log.module_num 
IS 'Identities module within a give mutli-step process.'; 

COMMENT ON COLUMN pfsawh_process_log.step_num 
IS 'Identities steps within a give mutli-step process.'; 

COMMENT ON COLUMN pfsawh_process_log.process_start_date 
IS 'When the process started.';

COMMENT ON COLUMN pfsawh_process_log.process_end_date 
IS 'When the process ended.';

COMMENT ON COLUMN pfsawh_process_log.process_status_code  
IS 'Processing status of the process. ';

COMMENT ON COLUMN pfsawh_process_log.process_recid 
IS 'Primary key for facRptToCourtProcessLog records.';

COMMENT ON COLUMN pfsawh_process_log.sql_error_code 
IS 'Return status of the select statement executed.';

COMMENT ON COLUMN pfsawh_process_log.rec_read_int 
IS 'Number of records read by the process.';

COMMENT ON COLUMN pfsawh_process_log.rec_valid_int 
IS 'Number of records found to be valid by the process.';

COMMENT ON COLUMN pfsawh_process_log.rec_load_int 
IS 'Number of records loaded by the process.';

COMMENT ON COLUMN pfsawh_process_log.rec_inserted_int 
IS 'Number of records inserted by the process';

COMMENT ON COLUMN pfsawh_process_log.rec_selected_int 
IS 'Number of records selected by the process';

COMMENT ON COLUMN pfsawh_process_log.rec_updated_int 
IS 'Number of records updateded by the process';

COMMENT ON COLUMN pfsawh_process_log.rec_deleted_int 
IS 'Number of records deleteed by the process';

COMMENT ON COLUMN pfsawh_process_log.user_login_id 
IS 'The user id the this process was run under.';

COMMENT ON COLUMN pfsawh_process_log.message 
...
WHERE  table_name = UPPER('pfsawh_process_log');
...
LEFT OUTER JOIN user_tab_columns b ON b.table_name = UPPER('pfsawh_process_log') 
    AND a.column_name = b.column_name
WHERE    a.table_name = UPPER('pfsawh_process_log') 
...
BEGIN 
    ...
    INSERT INTO pfsawh_process_log (process_recid, process_status_code, message) 
