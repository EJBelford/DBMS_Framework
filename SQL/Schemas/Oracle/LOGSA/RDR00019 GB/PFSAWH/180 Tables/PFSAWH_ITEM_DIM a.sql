...
SELECT MAX(rec_id) FROM pfsawh_item_dim;
...
ALTER TABLE pfsawh_item_dim  
    DROP CONSTRAINT pk_pfsawh_item_dim_niin_mcn;        

ALTER TABLE pfsawh_item_dim 
	ADD CONSTRAINT pk_pfsawh_item_dim_niin_mcn 
    PRIMARY KEY 
        (
        niin, 
        mcn
...
DROP INDEX idx_pfsawh_item_dim_mcn;        

CREATE INDEX idx_pfsawh_item_dim_mcn 
    ON pfsawh_item_dim 
    (
    mcn
    );
...
COMMENT ON COLUMN pfsawh_item_dim.item_dim_id 
IS 'ITEM_DIM_ID - LIW/PFSAWH identitier for the item/part.'; 
...
COMMENT ON COLUMN pfsawh_item_dim.ui 
