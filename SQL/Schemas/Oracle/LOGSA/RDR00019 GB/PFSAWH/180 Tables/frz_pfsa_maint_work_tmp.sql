/*--*----|----*----|----*----|---- TEAM ITSS ----*----|----|----*----|----*---*/
--
--         NAME: frz_pfsa_maint_work_tmp 
--      PURPOSE: To calculate the desired information.
--
-- TABLE SOURCE: frz_pfsa_maint_work_tmp.sql 
--
...
-- DROP TABLE frz_pfsa_maint_work_tmp;
    
CREATE TABLE frz_pfsa_maint_work_tmp
(
...
COMMENT ON TABLE frz_pfsa_maint_work_tmp 
IS 'frz_pfsa_maint_work_tmp - This table documents all work performed by maintanenace personnel during a maintenance event.  Work not attributable to a specific person is aggregated';


COMMENT ON COLUMN frz_pfsa_maint_work_tmp.rec_id 
IS 'REC_ID - Primary, blind key of the pfsawh_item_sn_p_fact table.'; 

COMMENT ON COLUMN frz_pfsa_maint_work_tmp.source_rec_id 
IS 'SOURCE_REC_ID - Identifier to the orginial record received from the outside source.'; 

COMMENT ON COLUMN frz_pfsa_maint_work_tmp.pba_id 
IS 'PBA_ID - PFSAW identitier for a particular Performance Based Agreement.'; 
 
COMMENT ON COLUMN frz_pfsa_maint_work_tmp.physical_item_id 
IS 'PHYSICAL_ITEM_ID - Foreign key of the PFSAWH_ITEM_DIM table.'; 

COMMENT ON COLUMN frz_pfsa_maint_work_tmp.physical_item_sn_id 
IS 'PHYSICAL_ITEM_SN_ID - Foreign key of the PFSAWH_ITEM_SN_DIM table.'; 

COMMENT ON COLUMN frz_pfsa_maint_work_tmp.force_unit_id 
IS 'FORCE_UNIT_ID - Foreign key of the PFSAWH_FORCE_UNIT_DIM table.'; 

COMMENT ON COLUMN frz_pfsa_maint_work_tmp.mimosa_item_sn_id 
...
COMMENT ON COLUMN frz_pfsa_maint_work_tmp.MAINT_EV_ID 
...
COMMENT ON COLUMN frz_pfsa_maint_work_tmp.MAINT_TASK_ID 
IS 'MAINT_TASK_ID - The identifier that when combined with the MAINT_EV_ID creates a unique maintenance task id.';

COMMENT ON COLUMN frz_pfsa_maint_work_tmp.MAINT_WORK_ID 
...
COMMENT ON COLUMN frz_pfsa_maint_work_tmp.MAINT_WORK_MH 
IS 'MAINT_WORK_MH - The number of hours the maintenance work took to complete.';

COMMENT ON COLUMN frz_pfsa_maint_work_tmp.MIL_CIV_KON 
...
COMMENT ON COLUMN frz_pfsa_maint_work_tmp.MOS 
...
COMMENT ON COLUMN frz_pfsa_maint_work_tmp.SPEC_PERSON 
IS 'SPEC_PERSON ? Specific person who performed the work.';

COMMENT ON COLUMN frz_pfsa_maint_work_tmp.REPAIR 
IS 'REPAIR - Item was repaired.  Values should be Y - Yes, N - No, ? - Unknown';

COMMENT ON COLUMN frz_pfsa_maint_work_tmp.MOS_SENT 
...
COMMENT ON COLUMN frz_pfsa_maint_work_tmp.STATUS 
IS 'STATUS - The status of the record.  Values are Q/R/P or D';

COMMENT ON COLUMN frz_pfsa_maint_work_tmp.LST_UPDT 
IS 'LST_UPDT - The date/time stamp the record was last updated';

COMMENT ON COLUMN frz_pfsa_maint_work_tmp.UPDT_BY 
...
COMMENT ON COLUMN frz_pfsa_maint_work_tmp.FRZ_INPUT_DATE 
IS 'FRZ_INPUT_DATE - ';

COMMENT ON COLUMN frz_pfsa_maint_work_tmp.FRZ_INPUT_DATE_ID 
IS 'FRZ_INPUT_DATE_ID - ';

COMMENT ON COLUMN frz_pfsa_maint_work_tmp.rec_frz_flag 
IS 'REC_FRZ_FLAG - Flag indicating if the record is frozen or not.  Values: N - Not frozen, Y - Frozen';

COMMENT ON COLUMN frz_pfsa_maint_work_tmp.frz_date 
IS 'FRZ_DATE - Additional control for REC_FRZ_FLAG indicating when the record was frozen.';

COMMENT ON COLUMN frz_pfsa_maint_work_tmp.insert_by 
IS 'INSERT_BY - Reports who initially created the record.';

COMMENT ON COLUMN frz_pfsa_maint_work_tmp.insert_date 
IS 'INSERT_DATE - Reports when the record was initially created.';

COMMENT ON COLUMN frz_pfsa_maint_work_tmp.update_by 
IS 'UPDATE_BY - Reports who last updated the record.';

COMMENT ON COLUMN frz_pfsa_maint_work_tmp.update_date 
IS 'UPDATE_DATE - Reports when the record was last updated.';

COMMENT ON COLUMN frz_pfsa_maint_work_tmp.delete_flag 
IS 'DELETE_FLAG - Flag indicating if the record can be deleted.';

COMMENT ON COLUMN frz_pfsa_maint_work_tmp.delete_date 
IS 'DELETE_DATE - Additional control for DELETE_FLAG indicating when the record was marked for deletion.';

COMMENT ON COLUMN frz_pfsa_maint_work_tmp.hidden_flag 
IS 'HIDDEN_FLAG - Flag indicating if the record should be hidden from the general user in things like drop-down lists.';

COMMENT ON COLUMN frz_pfsa_maint_work_tmp.hidden_date 
IS 'HIDDEN_DATE - Addition control for HIDDEN_FLAG indicating when the record was hidden.';

COMMENT ON COLUMN frz_pfsa_maint_work_tmp.DELETE_by 
IS 'DELETE_BY - Reports who deleted the record.';

COMMENT ON COLUMN frz_pfsa_maint_work_tmp.HIDDEN_by 
...
WHERE  table_name = UPPER('frz_pfsa_maint_work_tmp'); 
*/
...
LEFT OUTER JOIN user_tab_columns b ON b.table_name = UPPER('frz_pfsa_maint_work_tmp') 
    AND  a.column_name = b.column_name
WHERE    a.table_name = UPPER('frz_pfsa_maint_work_tmp') 
