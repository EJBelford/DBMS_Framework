/*--*----|----*----|----*----|---- TEAM ITSS ----*----|----|----*----|----*---*/
--
--         NAME: frz_pfsa_maint_items_tmp 
--      PURPOSE: To calculate the desired information.
--
-- TABLE SOURCE: frz_pfsa_maint_items_tmp.sql 
--
...
-- DROP TABLE frz_pfsa_maint_items_tmp;
...
--    
    ...
    PRIORITY                         NUMBER, 
    DOC_NO                           VARCHAR2(14),
    DOC_NO_EXPAND                    VARCHAR2(18),
    PART_UID                         VARCHAR2(78), 
--     
    STATUS                           VARCHAR2(1), 
    LST_UPDT                         DATE,
    UPDT_BY                          VARCHAR2(30), 
...
COMMENT ON TABLE frz_pfsa_maint_items_tmp 
IS 'FRZ_PFSA_MAINT_ITEMS_TMP - This table documents all items used/consumed during a maintenance event';


COMMENT ON COLUMN frz_pfsa_maint_items_tmp.rec_id 
IS 'REC_ID - Primary, blind key of the pfsawh_item_sn_p_fact table.'; 

COMMENT ON COLUMN frz_pfsa_maint_items_tmp.source_rec_id 
IS 'SOURCE_REC_ID - Identifier to the orginial record received from the outside source.'; 

COMMENT ON COLUMN frz_pfsa_maint_items_tmp.pba_id 
IS 'PBA_ID - PFSAW identitier for a particular Performance Based Agreement.'; 
 
COMMENT ON COLUMN frz_pfsa_maint_items_tmp.physical_item_id 
IS 'PHYSICAL_ITEM_ID - Foreign key of the PFSAWH_ITEM_DIM table.'; 

COMMENT ON COLUMN frz_pfsa_maint_items_tmp.physical_item_sn_id 
IS 'PHYSICAL_ITEM_SN_ID - Foreign key of the PFSAWH_ITEM_SN_DIM table.'; 

COMMENT ON COLUMN frz_pfsa_maint_items_tmp.force_unit_id 
IS 'FORCE_UNIT_ID - Foreign key of the PFSAWH_FORCE_UNIT_DIM table.'; 

COMMENT ON COLUMN frz_pfsa_maint_items_tmp.mimosa_item_sn_id 
...
COMMENT ON COLUMN frz_pfsa_maint_items_tmp.MAINT_EV_ID 
...
COMMENT ON COLUMN frz_pfsa_maint_items_tmp.MAINT_TASK_ID 
IS 'MAINT_TASK_ID - The identifier that when combined with the MAINT_EV_ID creates a unique maintenance task id.';

COMMENT ON COLUMN frz_pfsa_maint_items_tmp.MAINT_ITEM_ID 
...
COMMENT ON COLUMN frz_pfsa_maint_items_tmp.CAGE_CD 
...
COMMENT ON COLUMN frz_pfsa_maint_items_tmp.PART_NUM 
...
COMMENT ON COLUMN frz_pfsa_maint_items_tmp.status 
IS 'STATUS - The status of the record in question.';

COMMENT ON COLUMN frz_pfsa_maint_items_tmp.updt_by 
IS 'UPDT_BY - The date/timestamp of when the record was created/updated.';

COMMENT ON COLUMN frz_pfsa_maint_items_tmp.lst_updt 
IS 'LST_UPDT - Indicates either the program name or user ID of the person who updated the record.';

COMMENT ON COLUMN frz_pfsa_maint_items_tmp.FRZ_INPUT_DATE 
IS 'FRZ_INPUT_DATE - ';

COMMENT ON COLUMN frz_pfsa_maint_items_tmp.FRZ_INPUT_DATE_ID 
IS 'FRZ_INPUT_DATE_ID - ';

COMMENT ON COLUMN frz_pfsa_maint_items_tmp.rec_frz_flag 
IS 'REC_FRZ_FLAG - Flag indicating if the record is frozen or not.  Values: N - Not frozen, Y - Frozen';

COMMENT ON COLUMN frz_pfsa_maint_items_tmp.frz_date 
IS 'FRZ_DATE - Additional control for REC_FRZ_FLAG indicating when the record was frozen.';

COMMENT ON COLUMN frz_pfsa_maint_items_tmp.insert_by 
IS 'INSERT_BY - Reports who initially created the record.';

COMMENT ON COLUMN frz_pfsa_maint_items_tmp.insert_date 
IS 'INSERT_DATE - Reports when the record was initially created.';

COMMENT ON COLUMN frz_pfsa_maint_items_tmp.update_by 
IS 'UPDATE_BY - Reports who last updated the record.';

COMMENT ON COLUMN frz_pfsa_maint_items_tmp.update_date 
IS 'UPDATE_DATE - Reports when the record was last updated.';

COMMENT ON COLUMN frz_pfsa_maint_items_tmp.delete_flag 
IS 'DELETE_FLAG - Flag indicating if the record can be deleted.';

COMMENT ON COLUMN frz_pfsa_maint_items_tmp.delete_date 
IS 'DELETE_DATE - Additional control for DELETE_FLAG indicating when the record was marked for deletion.';

COMMENT ON COLUMN frz_pfsa_maint_items_tmp.DELETE_by 
IS 'DELETE_BY - Reports who deleted the record.';

COMMENT ON COLUMN frz_pfsa_maint_items_tmp.hidden_flag 
IS 'HIDDEN_FLAG - Flag indicating if the record should be hidden from the general user in things like drop-down lists.';

COMMENT ON COLUMN frz_pfsa_maint_items_tmp.hidden_date 
IS 'HIDDEN_DATE - Addition control for HIDDEN_FLAG indicating when the record was hidden.';

COMMENT ON COLUMN frz_pfsa_maint_items_tmp.HIDDEN_by 
...
WHERE  table_name = UPPER('frz_pfsa_maint_items_tmp'); 
*/
...
LEFT OUTER JOIN user_tab_columns b ON b.table_name = UPPER('frz_pfsa_maint_items_tmp') 
    AND  a.column_name = b.column_name
WHERE    a.table_name = UPPER('frz_pfsa_maint_items_tmp') 
