...
BEGIN 
    ...
    daily_incr_maint_items_load ('GBelford'); 
...
CREATE OR REPLACE PROCEDURE daily_incr_maint_items_load 
    (
...
-- Limit the data pull from LIDB.PFSAW to x number of days/months. 
    ...
    ps_id_key := NVL(type_maintenance, 'daily_incr_maint_items_load');
...
/*----------------------------------------------------------------------------*/ 
    ...
    select max(insert_date) into my_insert_dt from pfsa_maint_items;
    select max(update_date) into my_update_dt from pfsa_maint_items; 
    ...
    INTO   pfsa_maint_items_tmp 
        ...
        cage_cd,        part_num,          niin,
        part_sn,        num_items,         cntld_exchng,
        removed,        failure,          
        lst_updt,       updt_by,
        heir_id,        priority,
        doc_no,         doc_no_expand,
        part_uid  
        )
    SELECT maint_ev_id, maint_task_id,        maint_item_id,
        cage_cd,        part_num,             niin,
        part_sn,        num_items,            cntld_exchng,
        removed,        failure,     
        lst_updt,       updt_by,
    ...
    WHERE -- added in last update clause   
           lst_updt > my_lst_updt;  
...
--    UPDATE pfsa_maint_items_tmp mi
--    SET    pba_id = (
...
--                        AND itm.physical_item_id = mi.physical_item_id 
--                    )
--    WHERE mi.pba_id = 1000000 
--        OR mi.pba_id IS NULL; 
