...
BEGIN 
    ...
    daily_incr_maint_work_load ('GBelford'); 
...
CREATE OR REPLACE PROCEDURE daily_incr_maint_work_load 
    (
...
-- Limit the data pull from LIDB.PFSAW to x number of days/months. 
    ...
    ps_id_key := NVL(type_maintenance, 'daily_incr_maint_work_load');
...
-- Call the MERGE PBA REF routine.  
    ...
    ls_current_process.pfsa_process := 'DAILY_INCR_MAINT_WORK_LOAD'; 
...
/*----------------------------------------------------------------------------*/ 
    ...
    select max(insert_date) into my_insert_dt from pfsa_maint_work;
    select max(update_date) into my_update_dt from pfsa_maint_work; 
