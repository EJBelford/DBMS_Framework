/*--*----|----*----|----*----|---- Team ITSS ----|----*----|----*----|----*---*/
--
--            SP Name: etl_reload_sn 
--            SP Desc: 
--
--      SP Created By: Gene Belford
--    SP Created Date: 20 November 2008 
--
--          SP Source: etl_reload_sn.sql 
--
/*--*----|----*----|----*----|----*----|----*----|----*----|----*----|----*---*/
--      SP Parameters: 
--              Input: 
...
BEGIN 
    ...
    etl_reload_sn ('GBelford', l_call_error); 
...
CREATE OR REPLACE PROCEDURE etl_reload_sn    
    (
...
v_fact_ld_sn_cntrl_start    pfsawh_process_control.process_control_value%TYPE 
    := NULL; 
v_fact_ld_sn_cntrl_cutoff   pfsawh_process_control.process_control_value%TYPE 
    := NULL; 
...
CURSOR sn_cur IS
    SELECT  sn.rec_id,  
        sn.physical_item_id, sn.niin, 
        sn.physical_item_sn_id, sn.serial_number, 
        sn.item_nomen_standard,
        sn.scheduled_load_date, sn.scheduled_load_date_id,
        sn.load_date, sn.load_date_id, sn.status
    FROM     pfsawh_fact_ld_sn_cntrl sn 
    WHERE   sn.status = 'W' 
        AND sn.load_date IS NULL 
        AND ( sn.scheduled_load_date IS NULL 
              OR sn.scheduled_load_date < SYSDATE )
    ORDER BY sn.physical_item_id;

sn_rec    sn_cur%ROWTYPE; 
...
BEGIN 
    ...
    ps_id_key := nvl(type_maintenance, 'etl_reload_sn');
...
-- Get the process control values from PFSAWH_PROCESS_CONTROL. 
    ...
    v_fact_ld_sn_cntrl_start  := fn_pfsawh_get_prcss_cntrl_val('v_fact_ld_sn_cntrl_start');
    v_fact_ld_sn_cntrl_cutoff := fn_pfsawh_get_prcss_cntrl_val('v_fact_ld_sn_cntrl_cutoff'); 
...
--            OR (sn_cur%ROWCOUNT > 2)
            ); 
            
        v_time_id := fn_time_to_time_id(SYSDATE);

        EXIT WHEN ((v_time_id < v_fact_ld_sn_cntrl_start) 
            OR (v_time_id > v_fact_ld_sn_cntrl_cutoff)
            ); 

        proc0.rec_read_int := NVL(proc0.rec_read_int, 0) + 1;
        proc1.rec_read_int := NVL(proc1.rec_read_int, 0) + 1; 
        
        v_physical_item_id := sn_rec.physical_item_id;
...
---        pr_get_pfsawh_earliest_rcdt_sn (v_physical_item_id, ps_id_key);   
        
        COMMIT; 

---        pr_create_pfsawh_fact_shel_sn (v_physical_item_id, ps_id_key); 
        
        COMMIT; 
    
---        etl_pfsa_equip_avail_sn (v_physical_item_id, ps_id_key); 
        
        COMMIT; 
        
---        etl_pfsa_usage_event_sn (v_physical_item_id, ps_id_key); 
        
        COMMIT; 
        
---        etl_pfsa_maint_sn (v_physical_item_id, ps_id_key); 

        COMMIT; 
    
        UPDATE pfsawh_fact_ld_sn_cntrl 
        SET    load_date  = SYSDATE,
               status     = 'C'             
        WHERE  rec_id = sn_rec.rec_id; 
