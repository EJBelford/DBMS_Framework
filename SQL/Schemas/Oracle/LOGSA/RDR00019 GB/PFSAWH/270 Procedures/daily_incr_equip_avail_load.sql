...
BEGIN 
    ...
    daily_incr_equip_avail_load ('GBelford'); 
...
CREATE OR REPLACE PROCEDURE daily_incr_equip_avail_load 
    (
...
-- Limit the data pull from LIDB.PFSAW to x number of days/months. 
    ...
    ps_id_key := NVL(type_maintenance, 'daily_incr_equip_avail_load');
...
-- Call the MERGE PBA REF routine.  
    ...
    ls_current_process.pfsa_process := 'DAILY_INCR_EQUIP_AVAIL_LOAD'; 
...
-- PBA_ID 
    ...
    UPDATE pfsa_equip_avail_tmp sn
    SET    pba_id = (
