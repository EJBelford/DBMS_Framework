...
--               v_processRecId -  
--                     v_processKey - Code value for which process is logging activity 
-- 
...
BEGIN
    ...
    IF v_debug > 0 THEN
        DBMS_OUTPUT.PUT_LINE('v_rec_Id: ' || v_rec_Id || ', ' || v_processRecId || ', ' || v_processKey);
    END IF;  
...
-- Get the IDENTITY of the new record to pass back out for updates to the log entry
        ...
        SELECT MAX(process_key) INTO v_rec_Id FROM GB_PFSAWH_Process_Log;                 
    ...
    EXCEPTION
        ...
        WHEN OTHERS THEN
		    ...
		    insert into std_pfsa_debug_tbl (ps_procedure, ps_oerr, ps_location, called_by, ps_id_key, ps_msg, msg_dt)
		            values (ps_procedure_name, ps_oerr, ps_location, v_userLoginId, ps_id_key, ps_msg, sysdate);
--          RAISE;
