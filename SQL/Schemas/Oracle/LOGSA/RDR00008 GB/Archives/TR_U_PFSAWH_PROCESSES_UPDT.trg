/*--*----|----*----|----*----|---- Team ITSS ----|----*----|----*----|----*---*/
--
--        NAME:    tr_u_pfsawh_processes_updt
--
--     PURPOSE:    To perform work as each row is updated. 
-- 
--      SOURCE:    tr_u_pfsawh_processes_updt.trg
--   
-- ASSUMPTIONS:
--
-- LIMITATIONS:
...
CREATE OR REPLACE TRIGGER tr_u_pfsawh_processes_updt
BEFORE UPDATE
...
v_update_date gb_pfsawh_processes.update_date%TYPE  := sysdate;
v_update_by   gb_pfsawh_processes.update_by%TYPE    := user;
...
END tr_u_pfsawh_processes_updt;
