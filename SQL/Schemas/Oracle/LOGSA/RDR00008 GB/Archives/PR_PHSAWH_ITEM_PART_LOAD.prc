/*<TOAD_FILE_CHUNK>*/
CREATE OR REPLACE PROCEDURE   pr_PHSAWH_item_part_load 
...
ps_procedure_name       VARCHAR2(30)  := 'pr_PHSAWH_item_part_load';
...
-- DELETE gb_pfsa_item_part_bld WHERE niin = '015148052'
    ...
    IF  l_max_src_rowcnt < 1 
        ...
        pr_PHSAWH_InsUpd_ProcessLog (s0_processRecId, s1_processKey, 
...
SELECT * FROM gb_pfsa_item_part_bld ORDER BY niin

SELECT DISTINCT record_source FROM gb_pfsa_item_part_bld

SELECT * FROM gb_pfsa_item_part_bld WHERE fsc = '2355' ORDER BY niin
...
SELECT 
DISTINCT LENGTH(TRIM(nsn)), NSN 
FROM     GCSSA_HR_ASSET@pfsawh.lidbdev 
WHERE    LENGTH(TRIM(nsn)) > 13 ORDER BY LENGTH(TRIM(nsn))
