...
--
    ...
    item_from_date_id             NUMBER              NULL , 
    item_to_date_id               NUMBER              NULL , 
...
SELECT i.item_identifier_type_id, 
       i.item_type_value, 
       i.physical_item_id,
       ii.physical_item_id  
FROM   gb_pfsa_pba_items_ref i 
LEFT OUTER JOIN gb_pfsawh_item_dim ii ON i.item_type_value = ii.niin
WHERE  i.physical_item_id = 0;  

UPDATE  gb_pfsa_pba_items_ref  
SET    physical_item_id = 
           ( 
           SELECT ii.physical_item_id  
           FROM   gb_pfsawh_item_dim ii 
           WHERE  gb_pfsa_pba_items_ref .item_type_value = ii.niin
           ) 
