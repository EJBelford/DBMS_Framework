...
--
--
CREATE TABLE pfsawh.pfsawh_item_sn_t_fact
    (
...
SELECT status, count(*) FROM pfsawh_item_sn_bld_fact GROUP BY status; 

SELECT * FROM pfsawh_item_sn_p_fact; 

SELECT * FROM pfsawh_item_sn_a_fact;

SELECT * FROM pfsawh_item_sn_l_fact; 

SELECT * FROM pfsawh_item_sn_t_fact;
...
--    FROM   pfsawh_date_dim dt
--    WHERE  dt.date_id = pt.item_date_from_id) AS date_from,
    ( SELECT dt.oracle_date 
    FROM   date_dim dt
    WHERE  dt.date_dim_id = pt.item_date_to_id) AS date_to,
...
WHERE  pt.physical_item_sn_id > 0 
    ...
    AND pt.item_date_from_id = dtd.date_dim_id 
    AND ir.pba_id = 1000007  
    AND pt.physical_item_sn_id = ir.physical_item_sn_id 
ORDER BY pt.physical_item_id, pt.physical_item_sn_id, pt.item_date_from_id; 
...
SELECT -- pf.physical_item_id, 
    ir.pba_id, 
    fn_pfsawh_get_item_dim_field(pf.physical_item_id, 'item_nomen_short') AS item_nomen_short, 
    fn_pfsawh_get_item_dim_field(pf.physical_item_id, 'niin ') AS niin, 
    ( SELECT sn.item_serial_number 
    FROM   pfsawh_item_sn_dim sn
    WHERE  sn.physical_item_sn_id = pf.physical_item_sn_id) AS serial_num,
    pf.item_date_from_id, 
    ( SELECT dt.oracle_date 
    FROM   date_dim dt
    WHERE  dt.date_dim_id = pf.item_date_from_id) AS date_from,
...
FROM   pfsawh_item_sn_p_fact pf, 
     pfsa_pba_items_ref ir 
WHERE  pf.physical_item_sn_id > 0 
    AND ir.pba_id = 1000007  
    AND pf.physical_item_sn_id = ir.physical_item_sn_id 
ORDER BY pf.physical_item_id, pf.physical_item_sn_id, pf.item_date_from_id; 
