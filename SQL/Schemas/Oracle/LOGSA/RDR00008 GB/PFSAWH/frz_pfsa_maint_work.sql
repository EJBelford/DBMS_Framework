...
DROP SEQUENCE frz_pfsa_maint_work_seq;

CREATE SEQUENCE frz_pfsa_maint_work_seq
    START WITH 1
    MINVALUE   1
--    MAXVALUE   9999999
    NOCYCLE
...
COMMENT ON COLUMN frz_pfsa_maint_work.phyiscal_item_id 
IS 'PHYSICAL_ITEM_ID - Foreign key of the PFSAWH_ITEM_DIM table.'; 

COMMENT ON COLUMN frz_pfsa_maint_work.phyiscal_item_sn_id 
...
--COMMENT ON COLUMN frz_pfsa_maint_work.active_date 
...
ALTER TABLE frz_pfsa_maint_work  
    DROP CONSTRAINT pk0_frz_pfsa_maint_work;        

ALTER TABLE frz_pfsa_maint_work  
    ADD CONSTRAINT pk0_frz_pfsa_maint_work 
    PRIMARY KEY 
    (
    rec_id
    );    
...
ALTER TABLE frz_pfsa_maint_work
    ADD CONSTRAINT idu_frz_pfsa_maint_work 
        UNIQUE 
            (
            MAINT_EV_ID, 
            MAINT_TASK_ID, 
...
ALTER TABLE frz_pfsa_maint_work 
    ADD (
        CONSTRAINT fk_frz_work_to_task 
        FOREIGN KEY 
            (
            MAINT_EV_ID, 
            MAINT_TASK_ID
            ) 
        REFERENCES pfsa_maint_task 
            (
            MAINT_EV_ID,
            MAINT_TASK_ID
            )
...
ALTER TABLE frz_pfsa_maint_work  
    DROP CONSTRAINT ck_frz_pfsa_maint_work_act_fl;        

ALTER TABLE frz_pfsa_maint_work  
    ADD CONSTRAINT ck_frz_pfsa_maint_work_act_fl 
    CHECK (rec_frz_flag='N' OR rec_frz_flag='Y');

ALTER TABLE frz_pfsa_maint_work  
    DROP CONSTRAINT ck_frz_pfsa_maint_work_del_fl;        

ALTER TABLE frz_pfsa_maint_work  
    ADD CONSTRAINT ck_frz_pfsa_maint_work_del_fl 
    CHECK (delete_flag='N' OR delete_flag='Y');

ALTER TABLE frz_pfsa_maint_work  
    DROP CONSTRAINT ck_frz_pfsa_maint_work_hd_fl;       

ALTER TABLE frz_pfsa_maint_work  
    ADD CONSTRAINT ck_frz_pfsa_maint_work_hd_fl 
    CHECK (hidden_flag='N' OR hidden_flag='Y');

ALTER TABLE frz_pfsa_maint_work  
    DROP CONSTRAINT ck_frz_pfsa_maint_work_status;        

ALTER TABLE frz_pfsa_maint_work  
    ADD CONSTRAINT ck_frz_pfsa_maint_work_status 
...
CREATE PUBLIC SYNONYM frz_pfsa_maint_work FOR pfsaw.frz_pfsa_maint_work; 

/*----- Grants-----*/

GRANT SELECT ON frz_pfsa_maint_work TO LIW_BASIC; 

GRANT SELECT ON frz_pfsa_maint_work TO LIW_RESTRICTED; 

GRANT SELECT ON frz_pfsa_maint_work TO S_PFSAW; 

-- GRANT SELECT ON frz_pfsa_maint_work TO MD2L043; 

-- GRANT SELECT ON frz_pfsa_maint_work TO S_LOGSA_WEBPROP; 

-- GRANT SELECT ON frz_pfsa_maint_work TO S_PBUSE; 

-- GRANT SELECT ON frz_pfsa_maint_work TO S_WEBPROP; 

GRANT SELECT ON frz_pfsa_maint_work TO C_PFSAW; 
...
SELECT * FROM frz_pfsa_maint_work; 
