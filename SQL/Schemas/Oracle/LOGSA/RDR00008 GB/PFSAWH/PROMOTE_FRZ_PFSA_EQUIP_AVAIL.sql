...
/*--*----|----*----|----*----|---- Team ITSS ----|----*----|----*----|----*---*/
--
--            SP Name: promote_frz_pfsa_equip_avail 
--            SP Desc: 
--
--      SP Created By: Gene Belford
--    SP Created Date: 23 April 2008 
--
--          SP Source: PROMOTE_FRZ_PFSA_EQUIP_AVAIL.sql 
--
/*--*----|----*----|----*----|----*----|----*----|----*----|----*----|----*---*/
--      SP Parameters: 
--              Input: 
...
-- If the v_rec_Id is NULL then we assume that a new log record is required
    ...
    IF s0_rec_Id IS NULL THEN
        ...
        SELECT pea.sys_ei_niin, pea.sys_ei_sn, MAX(pea.ready_date)  
        FROM   pfsa_equip_avail pea, 
               pfsa_pba_frz_dates_ref ppfdr 
        WHERE  pea.sys_ei_niin = ppfdr.item_type_value 
            AND pea.sys_ei_sn LIKE 'LA27%M' 
        GROUP BY pea.sys_ei_niin, pea.sys_ei_sn  
        ORDER BY pea.sys_ei_niin, pea.sys_ei_sn; 
        
        SELECT fn_date_to_date_id(pea.ready_date), ppfdr.*, '|', pea.* 
        FROM   pfsa_equip_avail pea, 
               pfsa_pba_frz_dates_ref ppfdr 
        WHERE  pea.sys_ei_niin = ppfdr.item_type_value 
            AND fn_date_to_date_id(pea.ready_date) BETWEEN ppfdr.frz_fr_date_id AND ppfdr.frz_cutoff_date_id;
