/*--*----|----*----|----*----|---- Team ITSS ----|----*----|----*----|----*---*/
--
--        NAME:    tr_u_pfsa_pba_type_ref_updt
--
--     PURPOSE:    To perform work as each row is updated. 
-- 
--      SOURCE:    tr_u_pfsa_pba_type_ref_updt.trg
--   
-- ASSUMPTIONS:
--
-- LIMITATIONS:
...
CREATE OR REPLACE TRIGGER tr_u_pfsa_pba_type_ref_updt
BEFORE UPDATE
ON     pfsa_pba_type_ref
FOR EACH ROW
DECLARE
...
v_update_date pfsa_pba_type_ref.update_date%TYPE  := sysdate;
v_update_by   pfsa_pba_type_ref.update_by%TYPE    := user;
...
END tr_u_pfsa_pba_type_ref_updt;
