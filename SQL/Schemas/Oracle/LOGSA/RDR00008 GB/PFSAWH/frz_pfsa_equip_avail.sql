...
DROP SEQUENCE frz_pfsa_equip_avail_seq;

CREATE SEQUENCE frz_pfsa_equip_avail_seq
    START WITH 1
    MINVALUE   1
--    MAXVALUE   9999999
    NOCYCLE
...
COMMENT ON COLUMN frz_pfsa_equip_avail.phyiscal_item_id 
IS 'PHYSICAL_ITEM_ID - Foreign key of the PFSAWH_ITEM_DIM table.'; 

COMMENT ON COLUMN frz_pfsa_equip_avail.phyiscal_item_sn_id 
...
--COMMENT ON COLUMN frz_pfsa_equip_avail.active_date 
...
ALTER TABLE frz_pfsa_equip_avail  
    DROP CONSTRAINT pk0_frz_pfsa_equip_avail;        

ALTER TABLE frz_pfsa_equip_avail  
    ADD CONSTRAINT pk0_frz_pfsa_equip_avail 
    PRIMARY KEY 
    (
    rec_id
    );    
...
ALTER TABLE frz_pfsa_equip_avail 
    ADD (
        CONSTRAINT PK1_frz_pfsa_equip_avail
        UNIQUE 
            (
            sys_ei_niin, 
            pfsa_item_id, 
...
--ALTER TABLE frz_pfsa_equip_avail  
--    DROP CONSTRAINT ck_frz_pfsa_equip_avail_act_fl;        

--ALTER TABLE frz_pfsa_equip_avail  
--    ADD CONSTRAINT ck_frz_pfsa_equip_avail_act_fl 
--    CHECK (active_flag='I' OR active_flag='N' OR active_flag='Y');

ALTER TABLE frz_pfsa_equip_avail  
    DROP CONSTRAINT ck_frz_pfsa_equip_avail_del_fl;        

ALTER TABLE frz_pfsa_equip_avail  
    ADD CONSTRAINT ck_frz_pfsa_equip_avail_del_fl 
    CHECK (delete_flag='N' OR delete_flag='Y');

ALTER TABLE frz_pfsa_equip_avail  
    DROP CONSTRAINT ck_frz_pfsa_equip_avail_hd_fl;       

ALTER TABLE frz_pfsa_equip_avail  
    ADD CONSTRAINT ck_frz_pfsa_equip_avail_hd_fl 
    CHECK (hidden_flag='N' OR hidden_flag='Y');

ALTER TABLE frz_pfsa_equip_avail  
    DROP CONSTRAINT ck_frz_pfsa_equip_avail_status;        

ALTER TABLE frz_pfsa_equip_avail  
    ADD CONSTRAINT ck_frz_pfsa_equip_avail_status 
...
CREATE UNIQUE INDEX PK1_frz_pfsa_equip_avail ON frz_pfsa_equip_avail
...
CREATE PUBLIC SYNONYM frz_pfsa_equip_avail FOR pfsaw.frz_pfsa_equip_avail; 

/*----- Grants-----*/

GRANT SELECT ON frz_pfsa_equip_avail TO LIW_BASIC; 

GRANT SELECT ON frz_pfsa_equip_avail TO LIW_RESTRICTED; 

GRANT SELECT ON frz_pfsa_equip_avail TO S_PFSAW; 

-- GRANT SELECT ON frz_pfsa_equip_avail TO MD2L043; 

-- GRANT SELECT ON frz_pfsa_equip_avail TO S_LOGSA_WEBPROP; 

-- GRANT SELECT ON frz_pfsa_equip_avail TO S_PBUSE; 

-- GRANT SELECT ON frz_pfsa_equip_avail TO S_WEBPROP; 

GRANT SELECT ON frz_pfsa_equip_avail TO C_PFSAW; 
...
BEGIN 
    ...
    SELECT ir.pba_id, pea.* 
    FROM   pfsa_equip_avail pea, 
           pfsa_pba_items_ref ir 
    WHERE  pea.sys_ei_niin = ir.item_type_value;
...
SELECT * FROM frz_pfsa_equip_avail; 
