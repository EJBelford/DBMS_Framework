CREATE OR REPLACE PROCEDURE PFSAWH.pr_mimosa_ins_item_dim 
    (
    in_rec_id                IN      NUMBER  
    )
...
--
    DELETE mimosa_item_dim WHERE status = 'P' /*AND lst_updt > '31-JAN-2008'*/; 
...
----- pfsa_sys_ei@pfsawh.lidbdev ------    
    ...
    LOOP
        ...
        IF LENGTH(LTRIM(RTRIM(ei_rec.sys_ei_niin))) = 9 THEN 
            ...
            VALUES
            (
            fn_pfsawh_get_dim_identity('MIMOSA_ITEM_DIM'), 
            'Y', 
            ei_rec.sys_ei_niin, 
            ei_rec.lin, 
