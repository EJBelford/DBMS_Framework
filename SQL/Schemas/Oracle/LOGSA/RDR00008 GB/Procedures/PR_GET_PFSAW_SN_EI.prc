CREATE OR REPLACE PROCEDURE PFSAWH.get_pfsaw_sn_ei
    (
    in_rec_Id                IN      NUMBER  
    )
...
/*--*----|----*----|----*----|---- Team ITSS ----|----*----|----*----|----*---*/
--
--            SP Name: get_pfsaw_sn_ei
--            SP Desc: 
--
--      SP Created By: Gene Belford
--    SP Created Date: 15 January 2008 
--
--          SP Source: get_pfsaw_sn_ei.prc
--
/*--*----|----*----|----*----|----*----|----*----|----*----|----*----|----*---*/
--      SP Parameters: 
--              Input: 
...
-- Date range logic would be nice to.  
    ...
    FROM   pfsawh_source_hist_ref 
    WHERE  source_table = 'PFSA_SN_EI';
    ...
    IF l_source_last_processed_date = NULL THEN
        l_run_flg := 1;
        s0_message := 'Never loaded ' || 'PFSA_SN_EI' || ' before.';
    ELSIF NVL(l_source_last_processed_date, '01-JAN-1900') < l_max_prd_lst_updt THEN 
        l_run_flg := 2;
        s0_message := 'Updates found for ' || 'PFSA_SN_EI' || '.';
    ELSE 
        l_run_flg := 0;
        s0_message := 'No updates need for ' || 'PFSA_SN_EI' || ' dimension.';
    END IF;
    ...
    IF l_run_flg > 0 THEN 
        ...
        MERGE 
        INTO   pfsawh_item_sn_dim item
        USING  (
               SELECT sys_ei_niin, sys_ei_sn, 
                      /*sn_item_state, ready_state, sys_ei_state,*/ 
                      status, lst_updt, updt_by/*, 
                      possessor*/ 
               FROM   pfsa_sn_ei 
               ) ei
            ON (item.item_niin = ei.sys_ei_niin 
                AND item.item_serial_number = ei.sys_ei_sn)   
        WHEN MATCHED THEN 
            UPDATE 
            SET  update_by         = 'get_pfsaw_sn_ei', 
                 update_date       = SYSDATE 
...
--                   gen_nomen, 
            ...
            VALUES ( 
                   fn_pfsawh_get_item_dim_id(ei.sys_ei_niin), 
