/*--*----|----*----|----*----|---- Team ITSS ----|----*----|----*----|----*---*/
--
--        NAME:    tr_u_pfsawh_item_dim_updt
--
--     PURPOSE:    To perform work as each row is updated. 
-- 
--      SOURCE:    tr_u_pfsawh_item_dim_updt.trg
--   
-- ASSUMPTIONS:
--
-- LIMITATIONS:
...
CREATE OR REPLACE TRIGGER tr_u_pfsawh_item_dim_updt
BEFORE UPDATE
ON     pfsawh_item_dim 
FOR EACH ROW
DECLARE
...
v_update_date pfsawh_item_dim.update_date%TYPE  := sysdate;
v_update_by   pfsawh_item_dim.update_by%TYPE    := user;
