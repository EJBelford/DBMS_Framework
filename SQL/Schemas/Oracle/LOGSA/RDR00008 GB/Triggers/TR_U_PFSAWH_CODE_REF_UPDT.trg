/*--*----|----*----|----*----|---- Team ITSS ----|----*----|----*----|----*---*/
--
--        NAME:    tr_u_pfsawh_code_ref_updt
--
--     PURPOSE:    To perform work as each row is updated. 
-- 
--      SOURCE:    tr_u_pfsawh_code_ref_updt.trg
--   
-- ASSUMPTIONS:
--
-- LIMITATIONS:
...
CREATE OR REPLACE TRIGGER tr_u_pfsawh_code_ref_updt
BEFORE UPDATE
ON     pfsawh_code_ref
FOR EACH ROW
DECLARE
...
v_update_date pfsawh_code_ref.update_date%TYPE  := sysdate;
v_update_by   pfsawh_code_ref.update_by%TYPE    := user;
