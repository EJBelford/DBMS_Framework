/*--*----|----*----|----*----|---- Team ITSS ----|----*----|----*----|----*---*/
--
--        NAME:    tr_u_pfsawh_location_updt
--
--     PURPOSE:    To perform work as each row is updated. 
-- 
--      SOURCE:    tr_u_pfsawh_location_updt.trg
--   
-- ASSUMPTIONS:
--
-- LIMITATIONS:
...
CREATE OR REPLACE TRIGGER tr_u_pfsawh_location_updt
BEFORE UPDATE
ON     pfsawh_location_dim
FOR EACH ROW
DECLARE
...
v_update_date pfsawh_location_dim.update_date%TYPE  := sysdate;
v_update_by   pfsawh_location_dim.update_by%TYPE    := user;
