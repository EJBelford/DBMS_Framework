/*--*----|----*----|----*----|---- Team ITSS ----|----*----|----*----|----*---*/
--
--            SP Name: pr_get_cbmwh_earliest_rc_dt_ni  
--            SP Desc: 
--
--      SP Created By: Gene Belford 
--    SP Created Date: 30 September 2008 
--
--          SP Source: pr_get_cbmwh_earliest_rc_dt_ni.sql 
--
/*--*----|----*----|----*----|----*----|----*----|----*----|----*----|----*---*/
--      SP Parameters: 
--              Input: 
...
BEGIN 
    ...
    pr_get_cbmwh_earliest_rc_dt_ni (141223, 'GBelford'); 
...
CREATE OR REPLACE PROCEDURE pr_get_cbmwh_earliest_rc_dt_ni    
    (
...
ps_procedure_name                std_cbmwh_debug_tbl.ps_procedure%TYPE  
    := 'pr_get_cbmwh_earliest_rc_dt_ni';  /*  */
...
v_niin                     cbmwh_item_dim.niin%TYPE; 
v_item_nomen_standard      cbmwh_item_dim.item_nomen_standard%TYPE; 

cv_physical_item_id        cbmwh_item_sn_dim.physical_item_id%TYPE; 

v_debug                    NUMBER        
     := 1;   -- Controls debug options (0 -Off)
...
--    SELECT * FROM cbmwh_item_dim WHERE wh_flag = 'Y' ORDER BY physical_item_id; 

--    SELECT bpseh.* FROM   bld_pfsa_sn_ei_hist@pfsaw.lidb bpseh;  
...
/*
    ...
    SELECT MIN(bpseh.event_dt_time) 
    FROM   bld_pfsa_sn_ei_hist@pfsaw.lidb bpseh 
    WHERE  bpseh.sys_ei_niin = '013285964'; 
...
*/
    ...
    SET    wh_earliest_fact_rec_dt = 
               ...
               SELECT MIN(TO_CHAR(bpseh.event_dt_time, 'DD-MON-YYYY')) 
               FROM   bld_pfsa_sn_ei_hist@pfsaw.lidb bpseh 
               WHERE  cisd.item_serial_number = bpseh.sys_ei_sn 
                   AND cisd.wh_earliest_fact_rec_dt > bpseh.event_dt_time
               )
    WHERE  wh_flag = 'Y'  
        AND cisd.physical_item_id = cv_physical_item_id; 
...
/* 
    ...
    SELECT pea.sys_ei_sn, MIN(pea.from_dt) 
    FROM   pfsa_equip_avail pea, 
        cbmwh_item_sn_dim cisd 
    WHERE  pea.physical_item_sn_id = cisd.physical_item_sn_id 
        AND pea.physical_item_id = 141223 
    GROUP BY pea.sys_ei_sn; 
...
*/
    ...
    SET    wh_earliest_fact_rec_dt = 
               ( 
               SELECT MIN(pea.from_dt)   
               FROM   pfsa_equip_avail pea
               WHERE  cisd.item_niin = pea.sys_ei_niin 
                   AND cisd.item_serial_number = pea.sys_ei_sn 
                   AND NVL(cisd.wh_earliest_fact_rec_dt, '01-JAN-1900') < pea.from_dt
               )
    WHERE  wh_flag = 'Y'  
...
END; -- end of PR_GET_CBMWH_EARLIEST_RC_DT_NI procedure     
/
