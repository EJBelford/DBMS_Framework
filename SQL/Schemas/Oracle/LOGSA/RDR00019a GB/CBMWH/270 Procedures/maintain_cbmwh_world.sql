CREATE OR REPLACE PROCEDURE maintain_cbmwh_world    
    (
...
/*--*----|----*----|----*----|---- Team ITSS ----|----*----|----*----|----*---*/
--
--            SP Name: maintain_cbmwh_world 
--            SP Desc: 
--
--      SP Created By: Gene Belford
--    SP Created Date: 23 October 2008 
--
--          SP Source: maintain_cbmwh_world.sql 
-- 
/*--*----|----*----|----*----|----*----|----*----|----*----|----*----|----*---*/
-- 
...
--                                     support
-- 23DEC08 - GB  -           -      - Added batch id to support integration 
--                                     of NIIN and SN reload  
-- 12JAN09 - GB  -           -      - Changed message at EOF from  
--                 "End of Message" to "Sensitive But Unclassified"  
-- 12JAN09 - GB  - RDR00019b -      - Added cursor loop to include log info   
...
e_message             VARCHAR2(2000); -- any string 

e_cr_lf_str                      VARCHAR2(3)
    := ' ' || CHR(10) || CHR(13);
e_cr_str                         VARCHAR2(3)
    := ' ' || CHR(13);
...
-- Added batch id to support integration of NIIN and SN reload  
   ...
   proc0.process_batch_id := fn_cbmwh_get_prcss_run_id;
   proc1.process_batch_id := proc0.process_batch_id; 
   
   UPDATE cbmwh_process_log 
   SET    process_batch_id = proc0.process_batch_id 
   WHERE  process_key = proc0.process_batch_id; 
...
-- E-mail setup  
    ...
    IF e_user_env_db_name = 'LIWWH' THEN 
        e_user_env_server_host_nm := 'CBMWH@LIWWH';
        ...
        e_subject := 'CBMWH Production e-mail message - ' || e_user_env_db_name;      
    ELSIF e_user_env_db_name = 'LIWWHTST' THEN 
        e_user_env_server_host_nm := 'CBMWH@LIWWHTST';
        ...
        e_subject := 'CBMWH Test e-mail message - ' || e_user_env_db_name;      
    ELSIF e_user_env_db_name = 'LIWWHDEV' THEN 
        e_user_env_server_host_nm := 'CBMWH@LIWWHDEV';
        ...
        e_subject := 'CBMWH Development e-mail message - ' || e_user_env_db_name;      
    ELSE 
...
-- Cleanup the process log file - "PFSAWH.PFSAWH_PROCESS_LOG".
    ...
    ps_location := 'Cln Log';
    ls_current_process.cbmwh_process := 'CALL PR_PFSAWH_LOG_CLEANUP'; 
  
-- get the run criteria from the pfsa_processes table for the last run of 
-- the SUB process 
  
    IF run_today('pr_cbmwh_log_cleanup') THEN 
...
-- Cleanup the debug log file - "PFSAWH.STD_PFSA_DEBUG_TBL".
    ...
    ls_current_process.cbmwh_process := 'CALL PR_PFSAWH_DEBUGTBL_CLEANUP'; 
  
-- get the run criteria from the pfsa_processes table for the last run of 
-- the SUB process 
  
    IF run_today('pr_cbmwh_debugtbl_cleanup') THEN 
...
-- Housekeeping for the SUB process 
    ...
    ps_location := 'ETL';
    ls_current_process.cbmwh_process := 'CALL ETL_PFSAWH_WORLD'; 
...
-- Housekeeping for the SUB process 
    ...
    ps_location := 'PBA';
    ls_current_process.cbmwh_process := 'CALL ETL_PFSA_PBA'; 
...
-- Housekeeping for the SUB process 
    ...
    ls_current_process.cbmwh_process := 'CALL ETL_DAILY_INCR_LOAD'; 
...
-- Housekeeping for the SUB process 
    ...
    ls_current_process.cbmwh_process := 'CALL ETL_DAILY_INCR_FACT_LOAD'; 
...
-- Housekeeping for the SUB process 
    ...
    ls_current_process.cbmwh_process := 'CALL ETL_DAILY_FRZ_INCR_LOAD'; 
...
-- Housekeeping for the SUB process 
    ...
    ls_current_process.cbmwh_process := 'CALL ETL_RELOAD_NIIN'; 
...
-- Housekeeping for the SUB process 
    ...
    ps_location := 'Reld SN';
    ls_current_process.cbmwh_process := 'ETL_RELOAD_SN'; 
  
    ps_status := ps_location;
  
    IF run_today('etl_reload_sin') THEN 
        v_debug := v_debug; 
...
-- Housekeeping for the SUB process 
    ...
    ls_current_process.cbmwh_process := 'CALL PR_UPD_PFSA_FCS_SYS_DATES'; 
...
-- Housekeeping for the SUB process 
    ...
    ls_current_process.cbmwh_process := 'CALL CLEANUP_WH_WEEKLY'; 
...
-- 12JAN09 - RDR00019b - Start 
-- Added cursor loop to include log info in the e-mail message.   

--        ps_location := 'PFSAWH 60';            -- For std_pfsawh_debug_tbl logging. 
    
    v_msg := 'PROCESS STEPS THAT FAILED TO COMPLETE' || e_cr_str || 
             '=====================================' || e_cr_str ||  
        'BATCH ID  PROCESS DESCRIPTION             PRC  MODULE  OTHER INFO ' || e_cr_str ||
        '========  ==============================  ===  ======  =============='; 
   
    FOR incmplt_prcss_rpt  
    IN  (
        SELECT log.process_batch_id AS batch_id, 
            p.process_description, 
            log.process_recid AS prc,  
            log.module_num AS mod_#,  
            '|', 
            TO_CHAR(log.process_start_date, 'YYYY-MM-DD HH24:MI:SS') AS started, 
            TO_CHAR(log.process_end_date, 'YYYY-MM-DD HH24:MI:SS') AS ended, 
            log.sql_error_code AS sql_err, 
            log.message --, log.user_login_id AS user_id
        ---    , '||', log.* 
        FROM cbmwh_process_log log 
        LEFT OUTER JOIN cbmwh_process_ref p ON log.process_recid = p.process_key 
        WHERE log.process_end_date IS NULL 
            AND proc0.process_batch_id <= log.process_key 
        ORDER BY log.process_key DESC  
        )
    LOOP 
    
--        proc0.rec_read_int := NVL(proc0.rec_read_int, 0) + 1;
--        proc1.rec_read_int := NVL(proc0.rec_read_int, 0) + 1; 
    
        IF v_debug > 0 THEN
            DBMS_OUTPUT.PUT_LINE
                (
                   'batch_id: '      || RPAD(incmplt_prcss_rpt.batch_id,6)  
                || ' process_desc: ' || incmplt_prcss_rpt.process_description  
                || ' prc: '          || incmplt_prcss_rpt.prc 
                || ' module_nun: '   || incmplt_prcss_rpt.mod_# 
                || ' message: '      || incmplt_prcss_rpt.message 
                );
        END IF;
        
        v_msg := SUBSTR(v_msg || '~' 
            || e_cr_str  
            || LPAD(NVL(incmplt_prcss_rpt.batch_id, '0'), 8, ' ') || '  ' 
            || RPAD(incmplt_prcss_rpt.process_description, 30, ' ') || '  ' 
            || LPAD(incmplt_prcss_rpt.prc, 3, ' ') || '  '
            || LPAD(incmplt_prcss_rpt.mod_#, 6, ' ') || '  '
            || SUBSTR(incmplt_prcss_rpt.message, 1, 40)  
            , 1, 1000); 
        
        COMMIT; 
    
    END LOOP; -- incmplt_prcss_rpt  

-- 12JAN09 - RDR00019b - Stop 
    ...
    e_message := 'Generated ' || TO_CHAR(SYSDATE, 'DD-MON-YYYY HH:MI:SS') || e_cr_lf_str 
        ...
        || 'Success e-mail message for batch # ' || proc0.process_batch_id 
        || ' from logsa.pfsa@conus.army.mil ' || e_cr_lf_str  
        || v_msg 
        || e_cr_lf_str
...
-- 12JAN09 - RDR00019b - Stop 
        ...
        e_message := 'Generated ' || TO_CHAR(SYSDATE, 'DD-MON-YYYY HH:MI:SS') || e_cr_lf_str 
            ...
            || e_user_env_db_name || e_cr_lf_str 
            || ps_procedure_name || e_cr_lf_str 
            || 'Exception e-mail message for batch # ' || proc0.process_batch_id  
            || ' from logsa.pfsa@conus.army.mil ' || e_cr_lf_str  
            || v_msg 
            || e_cr_lf_str
            || '/*----- Sensitive But Unclassified -----*/';   
...
END; -- end of maintain_cbmwh_world procedure 
/
