CREATE OR REPLACE PROCEDURE pr_cbmwh_insupd_processlog 
    (
    v_processRecId          IN      cbmwh_process_log.process_recid%TYPE, 
    v_processKey            IN      cbmwh_process_log.process_key%TYPE,
    v_moduleNum             IN      cbmwh_process_log.module_num%TYPE          := NULL,
    v_stepNum               IN      cbmwh_process_log.step_num%TYPE            := NULL,
    v_processStartDt        IN      cbmwh_process_log.process_Start_Date%TYPE  := NULL,
    v_processEndDt          IN      cbmwh_process_log.process_End_Date%TYPE    := NULL,
    v_processStatusCd       IN      cbmwh_process_log.process_Status_Code%TYPE := NULL,
    v_sqlErrorCode          IN      cbmwh_process_log.sql_Error_Code%TYPE      := NULL,
    v_recReadInt            IN      cbmwh_process_log.rec_Read_Int%TYPE        := NULL,
    v_recValidInt           IN      cbmwh_process_log.rec_Valid_Int%TYPE       := NULL,
    v_recLoadInt            IN      cbmwh_process_log.rec_Load_Int%TYPE        := NULL,
    v_recInsertedInt        IN      cbmwh_process_log.rec_Inserted_Int%TYPE    := NULL,
    v_recMergedInt          IN      cbmwh_process_log.rec_Merged_Int%TYPE      := NULL,
    v_recSelectedInt        IN      cbmwh_process_log.rec_Selected_Int%TYPE    := NULL,
    v_recUpdatedInt         IN      cbmwh_process_log.rec_Updated_Int%TYPE     := NULL,
    v_recDeletedInt         IN      cbmwh_process_log.rec_Deleted_Int%TYPE     := NULL,
    v_userLoginId           IN      cbmwh_process_log.user_Login_Id%TYPE       := '',
    v_message               IN      cbmwh_process_log.message%TYPE             := '', 
    v_rec_Id                IN OUT  cbmwh_process_log.process_RecId%TYPE  
    )
...
/*--*----|----*----|----*----|----*----|----*----|----*----|----*----|----*---*/
--
--         SP Name: pr_cbmwh_insupd_processlog
...
--
--       SP Source: pr_cbmwh_insupd_processlog.sql
--
/*--*----|----*----|----*----|----*----|----*----|----*----|----*----|----*---*/
--   SP Parameters: 
--           Input: For INSERT:
...
-- 12JAN09 - GB  -          -      - Copied from pr_pfsawh_insupd_processlog 
--
/*--*----|----*----|----*----|----*----|----*----|----*----|----*----|----*---*/
...
BEGIN    
    ...
    pr_cbmwh_insupd_processlog (v_processRecId, v_processKey, 
...
END pr_cbmwh_insupd_processlog;
/
