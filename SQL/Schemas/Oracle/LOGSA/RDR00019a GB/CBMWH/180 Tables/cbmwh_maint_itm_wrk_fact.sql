...
-- DROP TABLE cbmwh_maint_itm_wrk_fact;
    
CREATE TABLE cbmwh_maint_itm_wrk_fact 
(
...
COMMENT ON TABLE cbmwh_maint_itm_wrk_fact 
IS 'CBMWH_MAINT_ITM_WRK_FACT - '; 


COMMENT ON COLUMN cbmwh_maint_itm_wrk_fact.rec_id 
IS 'REC_ID - Sequence/identity for dimension/fact table.'; 

COMMENT ON COLUMN cbmwh_maint_itm_wrk_fact.tsk_begin_date_id 
IS 'TSK_BEGIN_DATE_ID - Foreign key to the PFSAWH_DATE_DIM records.';

COMMENT ON COLUMN cbmwh_maint_itm_wrk_fact.tsk_begin_time_id 
IS 'TSK_BEGIN_TIME_ID - Foreign key of the PFSAWH_TIME_DIM records.'; 

COMMENT ON COLUMN cbmwh_maint_itm_wrk_fact.tsk_end_date_id 
IS 'TSK_END_DATE_ID - Foreign key to the PFSAWH_DATE_DIM records.'; 

COMMENT ON COLUMN cbmwh_maint_itm_wrk_fact.tsk_end_time_id 
IS 'TSK_END_TIME_ID - Foreign key of the PFSAWH_TIME_DIM records.'; 

COMMENT ON COLUMN cbmwh_maint_itm_wrk_fact.physical_item_id 
IS 'PHYSICAL_ITEM_ID - Foreign key to the PFSAWH_ITEM_DIM table.'; 

COMMENT ON COLUMN cbmwh_maint_itm_wrk_fact.physical_item_sn_id 
IS 'PHYSICAL_ITEM_SN_ID - Foreign key to the PFSAWH_ITEM_SN_DIM table.'; 

COMMENT ON COLUMN cbmwh_maint_itm_wrk_fact.mimosa_item_sn_id 
...
COMMENT ON COLUMN cbmwh_maint_itm_wrk_fact.maint_ev_id_b 
IS 'MAINT_EV_ID_B - A PFSA generated key used to accomodate the multiple sources of maintenance data used in the metrics.  The structure used to build the key is dependent on the source.  LIDB maintenance data is a concatenation of the won and accept_dt.  AMAC source data is a concatenation of mwo and ac_serial_number.'; 

COMMENT ON COLUMN cbmwh_maint_itm_wrk_fact.maint_task_id 
IS 'MAINT_TASK_ID - The identifier that when combined with the MAINT_EV_ID creates a unique maintenance task id.'; 

COMMENT ON COLUMN cbmwh_maint_itm_wrk_fact.maint_work_id 
...
COMMENT ON COLUMN cbmwh_maint_itm_wrk_fact.maint_org 
...
COMMENT ON COLUMN cbmwh_maint_itm_wrk_fact.maint_uic 
IS 'MAINT_UIC - The UIC provided by the STAMIS system for the Maintenance Event.'; 

COMMENT ON COLUMN cbmwh_maint_itm_wrk_fact.maint_lvl_cd 
IS 'MAINT_LVL_CD - The maint lvl of the org.'; 

COMMENT ON COLUMN cbmwh_maint_itm_wrk_fact.elapsed_tsk_wk_tm 
...
COMMENT ON COLUMN cbmwh_maint_itm_wrk_fact.wrk_mos 
...
COMMENT ON COLUMN cbmwh_maint_itm_wrk_fact.wrk_spec_person 
IS 'WRK_SPEC_PERSON - SPEC_PERSON - Specific person who performed the work.'; 

COMMENT ON COLUMN cbmwh_maint_itm_wrk_fact.wrk_repair 
IS 'WRK_REPAIR - Item was repaired.  Values should be Y - Yes, N - No, ? - Unknown'; 

COMMENT ON COLUMN cbmwh_maint_itm_wrk_fact.wrk_mos_sent 
...
COMMENT ON COLUMN cbmwh_maint_itm_wrk_fact.status 
IS 'STATUS - The status of the record in question.';

COMMENT ON COLUMN cbmwh_maint_itm_wrk_fact.updt_by 
IS 'UPDT_BY - The date/timestamp of when the record was created/updated.';

COMMENT ON COLUMN cbmwh_maint_itm_wrk_fact.lst_updt 
IS 'LST_UPDT - Indicates either the program name or user ID of the person who updated the record.';

COMMENT ON COLUMN cbmwh_maint_itm_wrk_fact.active_flag 
IS 'ACTIVE_FLAG - Flag indicating if the record is active or not.';

COMMENT ON COLUMN cbmwh_maint_itm_wrk_fact.active_date 
IS 'ACTIVE_DATE - Additional control for active_Fl indicating when the record became active.';

COMMENT ON COLUMN cbmwh_maint_itm_wrk_fact.inactive_date 
IS 'INACTIVE_DATE - Additional control for active_Fl indicating when the record went inactive.';

COMMENT ON COLUMN cbmwh_maint_itm_wrk_fact.insert_by 
IS 'INSERT_BY - Reports who initially created the record.';

COMMENT ON COLUMN cbmwh_maint_itm_wrk_fact.insert_date 
IS 'INSERT_DATE - Reports when the record was initially created.';

COMMENT ON COLUMN cbmwh_maint_itm_wrk_fact.update_by 
IS 'UPDATE_BY - Reports who last updated the record.';

COMMENT ON COLUMN cbmwh_maint_itm_wrk_fact.update_date 
IS 'UPDATE_DATE - Reports when the record was last updated.';

COMMENT ON COLUMN cbmwh_maint_itm_wrk_fact.delete_flag 
IS 'DELETE_FLAG - Flag indicating if the record can be deleted.';

COMMENT ON COLUMN cbmwh_maint_itm_wrk_fact.delete_date 
IS 'DELETE_DATE - Additional control for DELETE_FLAG indicating when the record was marked for deletion.';

COMMENT ON COLUMN cbmwh_maint_itm_wrk_fact.hidden_flag 
IS 'HIDDEN_FLAG - Flag indicating if the record should be hidden from the general user in things like drop-down lists.';

COMMENT ON COLUMN cbmwh_maint_itm_wrk_fact.hidden_date 
...
-- WHERE  table_name = UPPER('cbmwh_maint_itm_wrk_fact'); 
...
--     ON  b.table_name  = UPPER('cbmwh_maint_itm_wrk_fact') 
--     AND a.column_name = b.column_name
-- WHERE    a.table_name = UPPER('cbmwh_maint_itm_wrk_fact') 
...
DROP DATABASE LINK cbmwh.lidbdev; 

CREATE DATABASE LINK cbmwh.lidbdev 
CONNECT TO pfsaw IDENTIFIED BY dave05 USING 'lidbdev.logsa.army.mil'; 

SELECT ev.* 
FROM   pfsa_maint_event@cbmwh.lidbdev ev 
...
BEGIN 
    ...
    v_niin    := '013285964';       -- TANK - M1A2 
    v_item_id := 141223; 

--    v_niin    := '015231316';       -- CARRIER XM1151P1 
--    v_item_id := 307543; 
    
--    v_niin    := '014360005';       -- FIGHTING VEH INF M2A3
--    v_item_id := 141143; 

--    v_niin    := '012823747';       -- HELICPTR CARGO MH-47E
--    v_item_id := 304600; 

    DELETE cbmwh_maint_itm_wrk_fact 
    WHERE  physical_item_id = v_item_id;
    
    COMMIT; 

    INSERT 
    INTO   cbmwh_maint_itm_wrk_fact 
        (
        tsk_begin_date_id, 
        tsk_begin_time_id, 
        tsk_end_date_id, 
...
--        pba_id, 
    ...
    SELECT 
        ...
        fn_cbmwh_get_item_dim_id(NVL(ev.sys_ei_niin, '000000000')) AS item_sys_ei_id,  
        fn_cbmwh_get_item_sn_dim_id(NVL(ev.sys_ei_niin, '000000000'), NVL(ev.sys_ei_sn, 0)) AS item_sn_ei_id, 
...
--           wk.*, tk.*, ev.* 
    FROM   pfsa_maint_event@cbmwh.lidbdev ev, 
           pfsa_maint_task@cbmwh.lidbdev tk,
           pfsa_maint_work@cbmwh.lidbdev wk 
    WHERE  ev.maint_ev_id = tk.maint_ev_id
        AND ev.maint_ev_id = wk.maint_ev_id 
        AND tk.maint_task_id = wk.maint_task_id 
        AND ev.dt_maint_ev_cmpl > TO_DATE('01-JAN-2008', 'DD-MON-YYYY')
--        AND ev.sys_ei_niin = v_niin 
--        AND rownum < 100 
...
SELECT * FROM cbmwh_maint_itm_wrk_fact; 
...
SELECT DISTINCT TO_CHAR(dt_maint_ev_cmpl, 'YYYY-MM-DD') 
FROM   pfsa_maint_event@cbmwh.lidbdev ev
ORDER BY TO_CHAR(dt_maint_ev_cmpl, 'YYYY-MM-DD') DESC; 

SELECT 
DISTINCT physical_item_id
FROM   cbmwh_maint_itm_wrk_fact
ORDER BY physical_item_id; 
